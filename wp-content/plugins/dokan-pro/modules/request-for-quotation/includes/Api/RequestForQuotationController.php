...
class RequestForQuotationController extends DokanRESTController {
    ...
    public function get_products( $request ) {
        $objects      = dokan()->product->all();
        $data_objects = [];
        foreach ( $objects->posts as $object ) {
        ...
        return $this->format_collection_response( $response, $request, 12 );
    }
    ...
    protected function prepare_data_for_response( $product, $request ) {
        ...
        return apply_filters( 'dokan_rest_prepare_product_object', $response, $product, $request );
    }
    ...
    public function create_request_quote( $request ) {
        ...
        unset( $params['id'], $params['product_ids'], $params['offer_product_quantity'], $params['offer_price'] );
    ...
    public function update_request_quote( $request ) {
        ...
        unset( $params['id'], $params['offer_product_quantity'], $params['product_ids'], $params['offer_price'] );
    ...
    protected function prepare_links( $object, $request ) {
        return [
            'self'       => [
                'href' => rest_url( sprintf( '/%s/%s/%d', $this->namespace, $this->base, is_callable( $object, 'get_id' ) ? $object->get_id() : $object->id ) ),
            ],
            'collection' => [
