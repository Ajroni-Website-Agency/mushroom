...
class Hooks {
    ...
    public function custom_add_to_quote_button() {
        ...
        foreach ( $this->quote_rules as $rule ) {
            ...
            if ( 'simple' === $product->get_type() ) {
                echo '<a href="javascript:void(0)" rel="nofollow" data-product_id="' . intval( $product->get_id() ) . '" data-product_sku="' . esc_attr( $product->get_sku() ) . '" class="button dokan_request_button add_to_cart_button product_type_' . esc_html( $product->get_type() ) . '">' . esc_attr( $rule->button_text ) . '</a>';
